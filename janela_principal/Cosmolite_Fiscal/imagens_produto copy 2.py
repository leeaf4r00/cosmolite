...
def get_product_info(gtin):
    ...
    if product:
        if gtin_var.get() == gtin:
            ...
            # Tentar carregar a imagem original antes de redimensionar
            try:
            ...
            # Redimensionar a imagem se muito grande
...
# Função para buscar informações usando Selenium
def get_product_data_selenium(gtin):
    driver = webdriver.Chrome()  # Inicializa o driver do Selenium
    url = f'https://cosmos.bluesoft.com.br/produtos/{gtin}'
...
# Função para buscar informações do produto usando Selenium em uma nova thread
