...
import subprocess
from database_login import create_connection
...
# Define as constantes para o arquivo main.py
DB_FILE = "users.db"
...
def autenticar_usuario():
    ...
    layout_login = [
        ...
        [sg.Text(WINDOW_TITLE, font=("Helvetica", 20),
                 justification="center", pad=((40, 40), (0, 40)))],
        [sg.Text(USERNAME_LABEL, size=(15, 1), justification="right"), sg.Input(
            key="-USERNAME-", size=(30, 1), font=("Helvetica", 14))],
        [sg.Text(PASSWORD_LABEL, size=(15, 1), justification="right"), sg.Input(
    ...
    while True:
        ...
        if event == LOGIN_BUTTON:
            ...
            if autenticar_usuario_db(username, password):
                ...
                sg.popup(INVALID_CREDENTIALS_MESSAGE)

        if event == REGISTER_BUTTON:
            window_login.close()
            sg.popup(FIRST_TIME_ACCESS_MESSAGE)
            cadastrar_usuario()
            return
...
def cadastrar_usuario():
    ...
    layout_cadastro = [
        ...
        [sg.Text(PASSWORD_LABEL, size=(15, 1), justification="right"), sg.Input(
            key="-PASSWORD-", size=(30, 1), font=("Helvetica", 14), password_char="*")],
        [sg.Text("Confirme sua senha:", size=(15, 1), justification="right"), sg.Input(
            key="-CONFIRM_PASSWORD-", size=(30, 1), font=("Helvetica", 14), password_char="*")],
        [sg.Button(REGISTER_BUTTON, size=(15, 1), pad=(10, 10))],
    ...
    while True:
        ...
        if event == REGISTER_BUTTON:
            ...
            if password != confirm_password:
                sg.popup("As senhas digitadas não correspondem. Tente novamente.")
                continue
            ...
            if cadastrar_usuario_db(username, password):
                ...
                sg.popup(USERNAME_IN_USE_MESSAGE)
...
def abrir_janela_principal():
    ...
    janela_principal_path = os.path.join(
        current_dir, "janela_principal", "janela_principal.py")
